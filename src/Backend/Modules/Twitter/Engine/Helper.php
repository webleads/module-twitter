...
namespace Backend\Modules\Twitter\Engine;
...
use TijsVerkoyen\Twitter\Twitter as Twitter;
...
class Helper
{
    ...
    public static function updateStatusCache($widgetIds = null)
    {
        ...
        foreach ($widgets as $widget) {
            ...
            if ($widget['oauth_token'] !== null && $widget['oauth_token_secret'] !== null) {
                ...
                try {
                    // get the tweets
                    $statuses = BackendTwitterHelper::getTwitterInstance()->statusesUserTimeline($widget['twitter_id'],
                        null, null, $numberOfItems, null, null, null, true);
                ...
                foreach ($statuses as $status) {
                    // filter by tag
                    if ($widget['tag'] !== null && !stripos($status['text'], '#' . $widget['tag'])) {
                        continue;
                    }
                ...
                BackendModel::getContainer()->get('database')->update('twitter_widgets',
                    array('last_synced_on' => BackendModel::getUTCDate()), 'id = ?', $widget['widget_id']);
            }
        }
    }
    ...
    public static function updateUsersCache($userIds = null)
    {
        ...
        if (empty($userIds)) {
            $userIds = BackendTwitterModel::getUserIds();
        }
